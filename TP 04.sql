...
SELECT * FROM ARTICLE 
INNER JOIN FOURNISSEUR ON ARTICLE.ID_FOU=FOURNISSEUR.ID WHERE FOURNISSEUR.ID = (SELECT ID FROM FOURNISSEUR WHERE NOM = 'Dubois & Fils'); 
...
SELECT * FROM BON WHERE DATE_CMDE BETWEEN '2019-02-01' AND '2019-04-05 12:00:00'; 

-- k. Sélectionnez les divers bons de commande qui contiennent des boulons
SELECT DISTINCT BON.NUMERO FROM BON, COMPO, ARTICLE 
WHERE BON.ID=COMPO.ID_BON AND COMPO.ID_ART=ARTICLE.ID AND DESIGNATION like '%boulon%'; 
...
SELECT BON.NUMERO AS num_commanden SUM(COMPO.QTE * ARTICLE.PRIX) as prix_total
FROM BON
INNER JOIN COMPO ON COMPO.ID_BON = BON.ID
INNER JOIN ARTICLE ON ARTICLE.ID = COMPO.ID_ART
